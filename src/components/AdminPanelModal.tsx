...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const handleUpdateTopupImage = (key: string, url: string) => {
    ...
    if (!cleanUrl) {
      handleResetTopupImage(key);
      return;
    }
    const aliasMap: Record<string, string[]> = {
      indonesia_uid: ['indonesia_uid', 'indonesia'],
      indonesia: ['indonesia_uid', 'indonesia'],
      monthly_bd: ['monthly_bd', 'monthly', 'monthly_offer'],
      monthly: ['monthly_bd', 'monthly', 'monthly_offer'],
      monthly_offer: ['monthly_bd', 'monthly', 'monthly_offer'],
      weekly_monthly_combo: ['weekly_monthly_combo', 'weekly_monthly_offer', 'weekly_monthly'],
      weekly_monthly_offer: ['weekly_monthly_combo', 'weekly_monthly_offer', 'weekly_monthly'],
      weekly_monthly: ['weekly_monthly_combo', 'weekly_monthly_offer', 'weekly_monthly'],
      weekly_bd: ['weekly_bd', 'weekly', 'weekly_offer'],
      weekly: ['weekly_bd', 'weekly', 'weekly_offer'],
      weekly_offer: ['weekly_bd', 'weekly', 'weekly_offer'],
      weekly_lite_bd: ['weekly_lite_bd', 'weekly_lite'],
      weekly_lite: ['weekly_lite_bd', 'weekly_lite'],
      idcode_bd: ['idcode_bd', 'idcode'],
      idcode: ['idcode_bd', 'idcode'],
      ff_ingame_airdrop: ['ff_ingame_airdrop', 'airdrop', 'special_airdrop'],
      airdrop: ['ff_ingame_airdrop', 'airdrop', 'special_airdrop'],
      levelup_pass_bd: ['levelup_pass_bd', 'levelup_bd', 'levelup_pass', 'levelup'],
      levelup_bd: ['levelup_pass_bd', 'levelup_bd', 'levelup_pass', 'levelup'],
      levelup: ['levelup_pass_bd', 'levelup_bd', 'levelup_pass', 'levelup'],
    };

    const keysToSet = aliasMap[key] || [key];
  ...
  const handleResetTopupImage = (key: string) => {
    ...
    const keysToDelete = aliasMap[key] || [key];
    const updated = { ...topupImages };
  ...
  const handleCategoryImageUpload = async (
    ...
    if (file.size > 15 * 1024 * 1024) {
      onToast('⚠️ ফাইলের সাইজ সর্বোচ্চ 15MB হতে হবে!');
      return;
    }

    onToast('⏳ ছবি কমপ্রেস ও প্রসেস করা হচ্ছে...');
    try {
      const compressedBase64 = await compressImageFile(file, 640, 640, 0.85);
      if (compressedBase64) {
        if (targetType === 'tournament') {
          handleUpdateTournamentImage(targetId, compressedBase64);
        } else {
          handleUpdateTopupImage(targetId, compressedBase64);
        }
      } else {
        onToast('❌ ছবি আপলোড ব্যর্থ হয়েছে! পুনরায় চেষ্টা করুন।');
      }
    } catch {
      onToast('❌ ছবি প্রসেস করতে ত্রুটি হয়েছে!');
    }
    e.target.value = '';
  };
                        ...
                        {t.type === 'deposit' && t.status === 'approved' && (
                          <div className="flex items-center">
                            <button
                              ...
                              onClick={() => {
                                onRejectTransaction(t.id);
                                onToast(`⚠️ ভুয়া ডিপোজিট বাতিল ও ইউজারের ব্যালেন্স রিভার্স করা হয়েছে!`);
                              }}
                              className="px-2.5 py-1.5 bg-rose-950/80 hover:bg-rose-900 border border-rose-500/50 text-rose-300 rounded-xl text-[11px] font-bold font-rajdhani flex items-center gap-1.5 transition cursor-pointer"
                              title="ভুয়া TrxID হলে ব্যালেন্স কেটে বাতিল করুন"
                            >
                              <ShieldAlert className="w-3.5 h-3.5 text-rose-400" />
                              <span>FRAUD REVOKE (ভুয়া TrxID বাতিল)</span>
                            </button>
                          </div>
                        )}
                      </div>
